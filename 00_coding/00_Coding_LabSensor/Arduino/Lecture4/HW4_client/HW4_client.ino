...
void loop() {
  ...
  if (client.connect(host, 80)) {//client connect to host
    ...
    if (push1 == LOW) {
      ...
      Serial.println("Humi");
      client.print("Humi"); // send string to server
      while (client.connected()) {
        if (client.available()) {
          String dataH = client.readStringUntil('\r'); //dataH for humidity from server
    ...
    if (push2 == LOW) {
      Serial.print("    * Send to Server: ");
      Serial.println("Temp");
      client.print("Temp");  // send string to server
      while (client.connected()) {
        if (client.available()) {
